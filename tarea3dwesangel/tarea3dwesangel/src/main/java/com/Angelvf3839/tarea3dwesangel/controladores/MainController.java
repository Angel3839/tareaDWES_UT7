...
public class MainController {
    ...
    public String guardarMensaje(@RequestParam("idEjemplar") Long idEjemplar, 
        ...
        if (sesionActual == null) {
            ...
            return "redirect:/MensajesForm?error=true";
        }
        ...
        if (perfil != Perfil.CLIENTE && perfil != Perfil.PERSONAL && perfil != Perfil.ADMIN) {
            System.out.println("Usuario sin permisos para enviar mensajes.");
        ...
        if (!serviciosMensaje.validarMensaje(mensajeTexto)) {
            ...
            redirectAttributes.addFlashAttribute("error", "El mensaje no es válido.");
            return "redirect:/MensajesForm?error=true";
        }

        Persona persona = serviciosPersona.buscarPorNombreDeUsuario(usuario);
        if (persona == null) {
            System.out.println("No se encontró la persona autenticada en la base de datos.");
